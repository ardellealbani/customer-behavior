...
def dist_oot(oot:pd.DataFrame):
    query4 = """
        select 
            Decile,
            count(score_etu) as number_of_cases,
            sum(gojek_solus) as number_of_gojek_solus,
            sum(grape_solus) as number_of_grape_solus,
            sum(switch_to_grape) as number_of_switch_to_grape,
            sum(switch_to_gojek) as number_of_switch_to_gojek
        FROM (
        select
          Decile,
          score_etu,
          case WHEN treatment='gojek' AND response='gojek' THEN 1 ELSE 0 END AS gojek_solus,
          case WHEN treatment='grape' AND response='grape' THEN 1 ELSE 0 END AS grape_solus,
          case WHEN treatment='gojek' AND response='grape' THEN 1 ELSE 0 END AS switch_to_grape,  
          case WHEN treatment='grape' AND response='gojek' THEN 1 ELSE 0 END AS switch_to_gojek  
        FROM oot
        ) a
        GROUP BY 1
        ORDER BY 1
                 """
    dist_oot = sqldf(query4)
    return dist_oot

def dist_train(training:pd.DataFrame):
    query4 = """
        select 
          Decile,
          count(score_etu) as number_of_cases,
        ...
        ORDER BY 1
                 """
    dist_train = sqldf(query4)
    return dist_train

def dist_test(testing_1k:pd.DataFrame):
    query5 = """
        select 
          Decile,
          count(score_etu) as number_of_cases,
